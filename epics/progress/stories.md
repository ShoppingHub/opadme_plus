...
```
story-12-01 → Layout Progress: grafico aggregato + TimeRangeSelector          ⏳ da fare
story-12-02 → MacroAreaSelector: filtro per macro-area                        ⏳ da fare
story-12-03 → Empty state e loading state                                     ⏳ da fare
```
...
BetonMe è un'app mobile-first di osservazione del benessere. Crea la schermata Progress (route `/progress`) che mostra l'andamento storico globale dell'utente.
...
- Selettore time range: pill `"30d"` · `"90d"` · `"365d"` — default `"30d"`
...
Continua Epic 12 di BetonMe. Aggiungi il selettore macro-area sotto il grafico in Progress.
...
Continua Epic 12 di BetonMe. Aggiungi gli stati speciali della schermata Progress.
